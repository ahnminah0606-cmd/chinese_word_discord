...
매주 하나의 핵심 중국어 회화 문장을 정하고, 해당 문장을 활용한 초급 미니 대화를 매일 Discord로 보내는 Python 스크립트입니다.

대화는 OpenAI Responses API로 생성합니다. 중국어 원문, 성조가 포함된 병음, 한글식 발음, 자연스러운 한국어 뜻을 함께 제공합니다.

## 동작 방식

- 한 주는 월요일부터 일요일까지입니다.
- 매주 첫 실행에서 아주 쉬운 완성형 회화 문장 하나를 선정합니다.
- 그 주에는 같은 핵심 회화 문장을 매일 반복하며 서로 다른 상황의 미니 대화를 만듭니다.
- 핵심 회화 문장은 반복할 수 있지만, 과거에 보낸 전체 대화 조합과 동일한 대화는 다시 보내지 않습니다.
- 시작 후 4주마다 표현의 폭을 조금씩 넓히지만, 최종 단계도 날씨·약속·식사·쇼핑·이동·기분을 짧게 말하는 아주 쉬운 생활 회화 수준으로 제한합니다.
- 몇 달이 지나도 어려운 어휘, 성어, 문어체, 격식체, 긴 문장과 복잡한 문법은 사용하지 않습니다.
...
성공하면 별도 출력 없이 종료합니다. 생성된 대화, 주간 핵심 회화와 현재 단계는 `chinese_word_discord_history.json`에 저장됩니다.
...
포함된 워크플로는 매일 오후 9시 58분(한국 시간)에 실행됩니다. 실행 후 갱신된 `chinese_word_discord_history.json`을 저장소에 자동 커밋해, 다음 실행에서도 과거 대화의 중복을 검사할 수 있게 합니다.
